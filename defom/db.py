...
def get_all_forests_tile_details():
    try:
        forest_det = list(db.forests.find(
            {}, {'forest_tiles.tile_id': 1, 'forest_tiles.infered_threat_class': 1}))
        return forest_det
    except Exception as e:
        return e
    ...
    try:
        if state == 1:
            ...
            for pred in latest_pred_in_id:
                pred_dict[pred['tile_id']] = {
                    'id': pred['_id'], 'res': pred['class_infered']}
            return pred_dict
    except Exception as e:
        return e
...
def get_forests_pred_bnd():
    try:
        forests = list(db.forests.find(
            {}, {'boundary': 1, 'forest_tiles.infered_threat_class': 1}))
        return forests
    except Exception as e:
        return e
...
def get_tile_view_id(forest_id, tile_ids, date):
    try:
        acc_tiles = list(db.forestTiles.find({"forest_id": forest_id, "tile_id": {
                         '$in': tile_ids}, 'save_time': date}, {'image': 1, 'tile_id': 1}))
        return acc_tiles
    except Exception as e:
        return e
...
def get_forest_tiles(forest_id, date_dt):
    try:
        acc_date = list(db.forestTiles.find({'forest_id': forest_id, "save_time": {
                         "$lte": date_dt}}, {'save_time': 1, '_id': 0}).sort("save_time", -1).limit(1))
        if acc_date != []:
            acc_tiles = db.forests.find_one(
                {'_id': forest_id}, {'forest_name': 1, 'forest_tiles': 1, 'location': 1})
            inf_list = list(db.forestTiles.find({'forest_id': forest_id, 'save_time': acc_date[0]['save_time']}, {
...
def getTileView(forest_id, tiles_id):
    try:
        id_dict = db.forests.find_one({"_id":  forest_id, 'forest_tiles.tile_id': tiles_id}, {
                                      'forest_tiles.tile_id': 1, 'forest_tiles.update_view_id': 1})
...
def get_forest_areas(forest_id):
    try:
        res = db.forests.find_one(
            {"_id": forest_id}, {'boundary': 1, 'sub_areas': 1})
        return res
    except Exception as e:
        return e
...
def save_forest_areas(forest_id, data):
    try:
        res = db.forests.update_one(
            {"_id": forest_id}, {'$push': {'sub_areas': data}})
        return res
    except Exception as e:
        return e
...
def get_forest_page_det(forest_id):
    try:
        res = db.forestPage.find_one({"forest_id": forest_id}, {
                                     "entire_forest_view": 0})
        return res
    except Exception as e:
        return e
...
def get_forest_entire_view(forest_id):
    try:
        bin_image = db.forestPage.find_one(
            {"forest_id": forest_id}, {"entire_forest_view": 1})
...
def get_all_forest_det():
    try:
        forests = list(db.forests.find(
            {}, {"boundary": 0, "forest_tiles": 0, "status": 0, "sub_areas": 0}))
        return forests
    except Exception as e:
        return e
...
def save_forest_admin(username, first_name, last_name, forest_id, hashed_password, phone, user_id):
    try:
        res = db.forestAdmins.insert_one({'username': username, 'first_name': first_name, 'last_name': last_name,
                                         'forest_id': forest_id, 'password': hashed_password, 'phone': phone, 'user_id': user_id, 'status': 'New'})
        return res
    except Exception as e:
        return e
...
def save_forest_officer(username, first_name, last_name, forest_id, hashed_password, phone, user_id):
    try:
        res = db.forestOfficers.insert_one({'username': username, 'first_name': first_name, 'last_name': last_name,
                                           'forest_id': forest_id, 'password': hashed_password, 'phone': phone, 'user_id': user_id, 'status': 'New'})
        return res
    except Exception as e:
        return e
...
def get_forest_officers():
    try:
        res = list(db.forestOfficers.find({}, {'_id': 1, 'username': 1,
                   'first_name': 1, 'last_name': 1, 'phone': 1, 'status': 1}))
        return res
    except Exception as e:
        return e
...
def get_forest_id(username):
    try:
        result = db.forestAdmins.find_one(
            {'username': username}, {'_id': 0, 'forest_id': 1})
        return result
    except Exception as e:
        return e
