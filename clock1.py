...
def class_inf(image_list):
    try:
        ...
        image_list = [cv2.resize(img, CLASS_INPUT_SHAPE, interpolate=cv2.INTER_AREA) for img in image_list]
...
def set_forest_view(acceptable_forests):
...
@sched.scheduled_job('cron', hour=14, name="get_forest_tiles")
def save_tiles_daily():
    ...
    for doc in dict_list:
        try:
                ...
                if forest_status != "new":
                    ...
                    for doc in documents:
                        ...
                        if valid_threat != []:
                            ...
                            query = UpdateOne({'_id': doc['forest_id'], 'forest_tiles.tile_id':doc['tile_id']}, {'$set' : {'forest_tiles.$.infered_threat_present' : True, 'inference_updated_date' : today, 'update_view_id': doc['_id']}})
                            update_requests_1.append(query)  
                else:
                    view_collectable = True
                    update_requests_1.append(UpdateOne({'_id': doc['forest_id']}, {'$set' : {'forest_status' : "active"}}))
                    for doc in documents:
                        query = UpdateOne({'_id': doc['forest_id'], 'forest_tiles.tile_id':doc['tile_id']}, {'$set' : {'forest_tiles.$.infered_threat_present' : True, 'inference_updated_date' : today, 'update_view_id': doc['_id']}})
                        update_requests_1.append(query)  
            ...
            try:
                ...
                acc_td_tiles = masking_tiles
                acc_ys_tiles = get_tile_view_id(forest_id, accessing_tiles, end_date)
    ...
    try:
        ...
        set_forest_view(updatable_forests)
